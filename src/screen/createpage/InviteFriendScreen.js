...
export default function InviteFriendScreen({navigation}) {
  ...
  //         setContactNAMEList(contactNAMEList => [
  //           ...contactNAMEList,
  //           eachcontact.familyName + ' ' + eachcontact.givenName,
  //         ]);
  ...
  //   //   apiaddress =
  //   //     'http://' + 'localhost' + ':8080/v1/friend/email/' + emailSet;
  //   // } else {
  //   //   apiaddress =
  //   //     'http://' + '192.168.12.94' + ':8080/v1/friend/email/' + emailSet;
  //   // }
  ...
  const selectFriend = index => {
    ...
    //     setIsPress(
    //       isPress.slice(0, index).concat([true].concat(isPress.slice(index + 1))),
    //     );
    //     console.log(isPress);
    //   } else {
    ...
    //       isPress
    //         .slice(0, index)
    //         .concat([false].concat(isPress.slice(index + 1))),
    //     );
    //   }
  };
  ...
  return (
    <SafeAreaView style={styles.mainSafeViewArea}>
      ...
      <ScrollView style={styles.contents}>
        <Text style={styles.friendHeadText}>
          약속에 함께할 친구를 선택해주세요.
        </Text>
        {DATA.map((item, index) => {
          return (
            <ListItem
              ...
              {isPress[index] ? (
                <Avatar
                  source={{
                    uri: 'https://cdn3.vectorstock.com/i/1000x1000/09/82/check-icon-vector-10850982.jpg',
                  }}
                />
              ) : (
                <></>
      ...
      <View style={styles.fixToText}>
        <TouchableOpacity
          ...
          onPress={() => {
            navigation.goBack();
            AsyncStorage.removeItem('selectedFriendList');
